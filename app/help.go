...
func (h helpTypeGeneral) toContent() string {
	content := lipgloss.JoinVertical(lipgloss.Left,
		...
		keyStyle.Render("W")+descStyle.Render("         - Switch active workspace (where new sessions land)"),
		keyStyle.Render("V")+descStyle.Render("         - Cycle the workspace view filter (All → ws1 → ws2 → All)"),
		keyStyle.Render("z")+descStyle.Render("         - Fold/unfold the workspace group containing the selection"),
		"",
